		...
		<div className="space-y-6 px-0">
			...
			<>
						...
						<div key={item.id} className="flex items-center justify-between border rounded-xl p-4 bg-white">
							<div className="flex items-center gap-4">
								...
								<div>
									...
									<div className="text-sm text-slate-600">₺{item.price.toFixed(2).replace('.', ',')} x {item.quantity} adet</div>
									<div className="text-sm text-slate-800 font-semibold mt-1">Ara Toplam: ₺{(item.price * item.quantity).toFixed(2).replace('.', ',')}</div>
								</div>
							</div>
				...
				<div className="flex items-center justify-end gap-3">
					...
					<button className="inline-flex items-center justify-center rounded-md bg-[#19262e] px-4 py-2 text-white shadow hover:bg-slate-900 transition-colors">Ödemeye Geç</button>
				</div>
			</>
		</div>
	)
